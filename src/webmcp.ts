...
export interface SignalWebMCPState {
	...
	getMessageCache: () => Map<
		string,
		{ isGroup: boolean; groupId?: string; from: string }
	>;
	isConnected: () => boolean;
}
...
async function getSignalStatus(): Promise<Record<string, unknown>> {
	...
	return {
		...
		registeredDevice: ownAccount
			? { deviceId: ownAccount.device ?? null }
			: null,
...
async function listSignalChats(): Promise<Record<string, unknown>> {
	...
	const chatMap = new Map<
		string,
		{ type: "dm" | "group"; messageCount: number }
	>();

	for (const msg of cache.values()) {
		const chatId =
			msg.isGroup && msg.groupId ? `group:${msg.groupId}` : msg.from;
...
async function getSignalMessageStats(): Promise<Record<string, unknown>> {
	...
	return {
		...
		dmConversations: Array.from(cache.values()).filter((m) => !m.isGroup)
			.length,
		groupConversations: Array.from(cache.values()).filter((m) => m.isGroup)
			.length,
	};
}
 ...
 */
export function getWebMCPHandlers(): Record<
	string,
